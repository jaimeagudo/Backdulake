...
class Backdulake {
  ...
  handleSaved(response) {
    ...
    if( ! this.id || _.isEmpty(this.model) ){
      console.warn("Backdulake.handleSaved: The model prior to the server response is empty, assumming the Server response will provide it!");
      this.model=response.data || response;
    }

    let id = (response.data && response.data.id) || response.id;
    if( ! id ){
    ...
    if(this.id && (this.id != id )){
      console.error("Backdulake.handleSaved: The id from the response + (" + id + ") doesn't match the current model one (" + this.model.id  + ")");
      return;
    }
