...
class Backdulake {
  ...
  handleSaved(response) {
    ...
    if( ! id ){
      console.error("No id pased to Backdulake.handleSaved, changes NOT persisted on the store");
      return;
    }

    if(this.model.id && (this.model.id != id )){
      console.error("The id passed to the Backdulake.handleSaved + (" + id + ") doesn't match the current model one (" + this.model.id  + ") pased to Backdulake.handleSave");
      return;
    }
